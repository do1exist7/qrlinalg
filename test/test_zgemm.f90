...
contains
  ...
  subroutine run_case(transa, transb, m, n, k, alpha, beta, padding, failures)
    ! Compare one valid ZGEMM call with a direct scalar reference product.
    ! The active output is checked with a precision-scaled error bound, while
    ! padded rows and columns must remain bitwise unchanged.
    character, intent(in) :: transa, transb
  ...
  subroutine run_beta_zero_nan_case(transa, transb, failures)
    ! Verify that beta=0 replaces undefined active values of C without reading
    ! them. Quiet NaNs expose an accidental read as a non-finite result.
    character, intent(in) :: transa, transb
    integer, intent(inout) :: failures
  ...
  subroutine fill_matrix(matrix, seed)
    ! Fill physical storage, including leading-dimension padding, with bounded
    ! deterministic complex values valid for every supported working kind.
  ...
  function matrix_element(matrix, transpose_mode, row, column) result(value)
    ! Return op(matrix)(row,column) using the BLAS transpose and conjugation
    ! conventions exercised by the direct reference product.
